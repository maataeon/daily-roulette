...
import { deleteItem, getItem, getItemColor, getItems, getSelectedItem, setSelectedItem } from "./itemList";
...
const cursorPosition = function () {
  ...
  if (newIndex !== position.index) {
    const item = getItem(position.index);
...
const stopRotateWheel = () => {
  ...
  if (getItems().length == 1) {
    ...
    playWink();
    setTimeout(() => deleteItem(item), 750);
  }
};
